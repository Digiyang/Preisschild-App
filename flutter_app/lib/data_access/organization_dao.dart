...
class OrganizationDao {
  ...
  OrganizationDao (int id, String name,
                    String category, String logoUrl,
                    String imageUrl, String description,
                    String shortDescription, int settingsId,
                    String locale, String welcomeText) {
    ...
    this.settings = SettingsDao(settingsId, locale, welcomeText);
    this.locations = [];
    this.orders = [];
    this.products = [];
  }
  ...
  static List<OrganizationDao> convert(String v) {
    ...
    for (String l in lines) {
      if (l.contains("|")) {
        ...
        org_details.add(OrganizationDao(int.parse(values[1].trim()), values[2].trim(),
                                        values[3].trim(), values[4].trim(),
                                        values[5].trim(), values[6].trim(),
                                        values[7].trim(), int.parse(values[8].trim()),
